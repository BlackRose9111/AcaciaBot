...
class servertime():
    ...
    def create(self):
        main.dbDictCursor.execute("INSERT INTO servertime (serverid, servertitle, serverbeforetext, serveraftertext, serverfooter, servertz, servercolor) VALUES (%s, %s, %s, %s, %s, %s, %s)", (self.serverid, self.servertitle, self.serverbeforetext, self.serveraftertext, self.serverfooter, self.servertz, self.servercolor))
    ...
    def update(self,**kwargs):
        ...
        if not self.checktzvalid(self.servertz) or self.servercolor < 0 or self.servercolor > 16777215:
            print("Invalid value")
            return False
        main.dbDictCursor.execute("UPDATE servertime SET serverid = %s, servertitle = %s, serverbeforetext = %s, serveraftertext = %s, serverfooter = %s, servertz = %s, servercolor = %s WHERE id = %s", (self.serverid, self.servertitle, self.serverbeforetext, self.serveraftertext, self.serverfooter, self.servertz, self.servercolor, self.id))
...
class usertime():
    ...
    def __init__(self,userid, id = None,usertitle = "Default", userbeforetext = "Default", useraftertext = "", userfooter = "Default Footer", usertz = "Europe/Istanbul", usercolor = main.rebeccapink):
        self.userid = userid
        self.id = id
    ...
    def create(self):
        main.dbDictCursor.execute("INSERT INTO usertime (userid, usertitle, userbeforetext, useraftertext, userfooter, usertz, usercolor) VALUES (%s, %s, %s, %s, %s, %s, %s)", (self.userid, self.usertitle, self.userbeforetext, self.useraftertext, self.userfooter, self.usertz, self.usercolor))
        self.id = main.dbDictCursor.lastrowid
    ...
    def update(self,**kwargs):
        ...
        if not self.checktzvalid(self.usertz) or self.usertz == "None" or self.usercolor < 0 or self.usercolor > 0xFFFFFF:
            print("Invalid value")
            return False
        main.dbDictCursor.execute("UPDATE usertime SET userid = %s, usertitle = %s, userbeforetext = %s, useraftertext = %s, userfooter = %s, usertz = %s, usercolor = %s WHERE id = %s", (self.userid, self.usertitle, self.userbeforetext, self.useraftertext, self.userfooter, self.usertz, self.usercolor, self.id))
...
class MemberTime():
    ...
    def __init__(self,serverid,userid ,id = None, userservertitle = "Default", userserverbeforetext = "Default", userserveraftertext = "", userserverfooter = "Default Footer", userservertz = "Europe/Istanbul", userservercolor = main.rebeccapink):
        self.id = id
        self.userid = userid
        self.serverid = serverid
